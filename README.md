...
there are 27301 images in the pron26 zip

To build,
```
docker build -t sg3 .
```
that makes an image named sg3

```
docker run --expose 9000 --network="host" -v $PWD:/workspace/ --shm-size=16gb --gpus all -it sg3 /bin/bash
```
that drops you into a command prompt in the container, mounting the current working directory to /workspace/

```
python train.py --outdir=./results-landscapes --cfg=stylegan3-t --data=landscapes.zip --gpus=1 --batch=4 --batch-gpu=4 --gamma=10.0 --mirror=1 --kimg=1000 --snap=2 --metrics=None --resume=latest.pkl
```

```
python train.py --outdir=./results-pron --cfg=stylegan3-t --data=pron-jan26.zip --gpus=1 --batch=32 --batch-gpu=4 --gamma=32 --mirror=1 --kimg=5000 --snap=2 --metrics=None --resume=./results-pron/00004-stylegan3-t-pron-jan26-gpus1-batch32-gamma32/network-snapshot-000368.pkl
```
